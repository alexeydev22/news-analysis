...
def build_architecture_diagram() -> Path:
    ...
    img.save(ASSETS_DIR / "architecture-diagram.png")
    return path
...
def add_contents(doc: Document) -> None:
    ...
    entries = [
        ("Введение", "3"),
        ("1. Теоретические основы анализа экономических новостей", "4"),
        ("2. Данные и постановка ML-задачи", "6"),
        ("3. Методы и модели", "9"),
        ("4. Архитектура программной системы", "12"),
        ("5. Реализация и пользовательский сценарий", "15"),
        ("6. Эксперимент и результаты", "20"),
        ("7. Направления улучшения", "24"),
        ("Заключение", "26"),
...
def economic_domain_paragraphs() -> list[str]:
    return [
        "Экономические новости отличаются от обычных новостных текстов тем, что их значение часто зависит от контекста показателя. Рост одного показателя может быть положительным, а рост другого - отрицательным. Например, рост прибыли компании обычно воспринимается позитивно, а рост инфляции или безработицы может указывать на ухудшение макроэкономической ситуации.",
        "Еще одна особенность домена состоит в наличии временного фактора. Новость может быть важна не только сама по себе, но и как часть последовательности сообщений. Повторяющиеся новости о снижении спроса, сокращении заказов или повышении ставки формируют более сильный сигнал, чем единичная публикация. Поэтому тематический прогноз по группам новостей является естественным расширением задачи классификации.",
        "В экономических текстах часто встречаются числовые показатели, названия компаний, индексы, регуляторы и отраслевые термины. Часть этих признаков важна для человека, но не всегда корректно используется простой моделью. Поэтому дальнейшее развитие системы может включать извлечение структурных признаков: сектора, тикера, даты, страны, показателя и направления изменения.",
        "Для языковой модели экономический домен также создает ограничения. Модель может хорошо формулировать связный текст, но без найденных источников она не гарантирует актуальность и проверяемость вывода. Поэтому в проекте языковая модель используется после retrieval и ML-анализа. Она не заменяет корпус новостей, а преобразует найденный контекст в понятный ответ.",
        "Такое понимание предметной области определяет структуру системы. Данные нужны не только для обучения, поиск нужен не только для интерфейса, классификация нужна не только для таблицы метрик, а языковая модель нужна не только для красивого текста. Все компоненты вместе решают одну прикладную задачу: дать пользователю краткий, проверяемый и объяснимый анализ экономических новостей.",
    ]
...
def system_flow_paragraphs() -> list[str]:
    return [
        "Полный путь обработки в системе начинается с загрузки новостного корпуса. На этом этапе данные приводятся к единой схеме: идентификатор новости, заголовок, полный текст, источник, дата публикации и целевая метка влияния. Такая унификация нужна для того, чтобы один и тот же корпус можно было использовать и в исследовательском ML-пайплайне, и в пользовательском RAG-сценарии.",
        "После подготовки новости индексируются для поиска. Индексация решает отдельную задачу: система должна находить документы не только по точному совпадению слов, но и по смысловой близости к вопросу пользователя. Поэтому retrieval-компонент является самостоятельной частью решения, а не вспомогательной функцией интерфейса.",
        "Когда пользователь задает вопрос, API Gateway передает запрос в диалоговый сценарий. Сначала retrieval-service выбирает релевантные новости, затем analysis-service оценивает их влияние с помощью ML-модели, после чего dialog-service формирует контекст для ответа. Внешняя языковая модель получает уже подготовленный контекст, а не исходный вопрос без ограничений.",
        "Такой порядок важен для надежности результата. Если языковая модель отвечает без источников, пользователь не может проверить, какие факты лежат в основе вывода. В реализованной системе ответ связан с конкретными документами: они показываются в интерфейсе вместе с оценками релевантности и ML-сигналами.",
    ]


def hyperparameter_paragraphs() -> list[str]:
    return [
        "Подбор гиперпараметров в работе рассматривается как способ проверить устойчивость выбранного baseline. Для TF-IDF-модели важны размер словаря, диапазон n-грамм, минимальная частота токена, максимальная доля документов для слишком частых слов и коэффициент регуляризации логистической регрессии.",
        "Автоматический подбор параметров может выполняться по сетке с использованием validation split. В качестве критерия целесообразно использовать macro F1, потому что он лучше отражает качество на всех классах при дисбалансе данных. Это соответствует постановке задачи: система должна распознавать не только самый частый класс, но и менее представленные negative/neutral-сигналы.",
        "Для embedding-logreg гиперпараметры связаны не только с классификатором, но и с представлением текста. В дальнейшем можно сравнивать разные embedding-модели, нормализацию векторов и параметры логистической регрессии. Для transformer-классификатора наиболее значимы learning rate, batch size, число эпох, max length и веса классов.",
        "В текущей версии проекта основной упор сделан на воспроизводимое сравнение трех классов моделей. Поэтому сложный подбор параметров вынесен в направления развития, а в основной эксперимент включены варианты, которые можно запустить локально и объяснить на защите без чрезмерного усложнения кода.",
    ]


def overfitting_paragraphs() -> list[str]:
    return [
        "Проверка переобучения выполняется через сравнение качества на validation и test, а также через анализ матрицы ошибок. Если модель показывает высокое качество на обучающих данных, но резко теряет macro F1 на тесте, это означает, что она запомнила особенности train split и плохо обобщает новые новости.",
        "В текущем проекте важна не только абсолютная метрика, но и стабильность выводов. TF-IDF + Logistic Regression является более устойчивым вариантом, потому что линейная модель на регуляризованных признаках меньше склонна к переобучению, чем маленький transformer при ограниченном режиме обучения.",
        "Дополнительным индикатором является интерпретация признаков. Если среди top-признаков появляются осмысленные экономические маркеры, например growth, higher, fell, lower, это подтверждает, что модель использует содержательные сигналы. Если бы top-признаки состояли из случайных артефактов, доверие к модели было бы ниже.",
    ]


def evaluation_protocol_paragraphs() -> list[str]:
    return [
        "Экспериментальный протокол в работе строится так, чтобы сравнение моделей было воспроизводимым. Все модели обучаются и оцениваются на одной и той же подготовленной выборке, используют одинаковый набор классов и возвращают результат в общем формате. Это исключает ситуацию, когда улучшение метрики связано не с моделью, а с изменением данных или правил оценки.",
        "Для корректной интерпретации результатов важно разделять validation и test. Validation используется для контроля настройки и выбора рационального варианта, а test должен оставаться финальной проверкой. Если многократно подбирать параметры по test, итоговая метрика перестанет отражать реальное обобщение модели на новых новостях.",
        "В ML-отчете рядом с метриками показывается матрица ошибок. Она нужна для ответа на вопрос, какие классы путаются чаще всего. Для экономических новостей такая информация особенно важна: нейтральное сообщение может содержать слова роста или падения, но не нести однозначного сигнала; негативная новость может быть написана формально и не содержать явных эмоциональных маркеров.",
        "Еще один элемент протокола - контроль времени инференса. В исследовательской задаче можно обучить тяжелую модель, но в диалоговой системе пользователь ожидает ответ без долгого ожидания. Поэтому модель оценивается не только по качеству, но и по скорости применения. Быстрый baseline имеет практическое преимущество, если его качество достаточно для текущего сценария.",
        "Сохранение результатов в MLflow делает эксперимент воспроизводимым. В дальнейшем можно сравнить новые версии признаков, параметры моделей и результаты на одном отчете. Для курсовой работы это важно, потому что демонстрирует не только разовый запуск, но и основу для дальнейшей исследовательской работы.",
    ]


def error_analysis_paragraphs() -> list[str]:
    return [
        "Типичные ошибки классификации связаны с тем, что экономический текст часто описывает несколько эффектов одновременно. Например, рост выручки компании является позитивным сигналом, но если он сопровождается ростом затрат или снижением маржинальности, итоговая оценка может быть менее очевидной. Простая модель на словах может зафиксировать только часть такого контекста.",
        "Часть ошибок возникает из-за различий между общим настроением текста и экономическим влиянием. Новость может быть написана нейтральным стилем, но описывать существенный негативный риск. Обратная ситуация также возможна: эмоционально окрашенный заголовок может не соответствовать реальному экономическому эффекту. Поэтому задача классификации влияния сложнее обычного sentiment analysis.",
        "Ошибки embedding-модели могут объясняться тем, что универсальные смысловые векторы сглаживают важные доменные слова. Для общего поиска это полезно, но для классификации влияния иногда важны конкретные маркеры: fell, lower, profit, approval, inflation. TF-IDF сохраняет такие признаки явно, поэтому на текущем корпусе показывает более сильный результат.",
        "Ошибки transformer-классификатора в текущей версии связаны не с непригодностью transformer-подхода, а с ограничениями выбранного легкого режима. Маленькая модель требует аккуратной настройки, достаточного числа эпох, контроля learning rate и работы с классами. Без этого она может либо недообучиться, либо запомнить поверхностные признаки train split.",
        "Практический вывод из анализа ошибок состоит в том, что улучшать нужно не только архитектуру модели. Значительную роль играют качество разметки, баланс классов, очистка дублей, длина входного текста и учет экономического контекста. Поэтому направления развития включают работу с данными наравне с подбором более сложных моделей.",
    ]


def validity_paragraphs() -> list[str]:
    return [
        "При интерпретации эксперимента необходимо учитывать угрозы валидности. Первая угроза связана с происхождением меток: если метка влияния получена автоматически или косвенно, она может не полностью совпадать с экспертной оценкой экономиста. В этом случае модель обучается не идеальной истине, а приближенной разметке.",
        "Вторая угроза связана с тематическим составом корпуса. Если в выборке много новостей о компаниях одного типа или о конкретных рыночных событиях, модель может переоценить признаки, характерные именно для этих тем. На новых источниках или в другом периоде распределение слов и событий может измениться.",
        "Третья угроза связана с языком и стилем новостей. Экономические публикации могут быть короткими заголовками, длинными аналитическими текстами или пресс-релизами. Эти форматы по-разному выражают влияние. Поэтому в дальнейшем полезно анализировать качество отдельно для заголовков, полных текстов и разных источников.",
        "Четвертая угроза связана с тем, что итоговая система объединяет несколько компонентов. Даже если классификатор работает приемлемо, слабый retrieval может ухудшить ответ. И наоборот, хороший поиск не компенсирует неверную оценку влияния. Поэтому в развитии проекта нужно оценивать не только отдельные модели, но и весь пользовательский сценарий.",
        "Эти ограничения не отменяют результата работы, но задают корректные рамки интерпретации. В курсовой работе важно показать не только достигнутые метрики, но и понимание того, почему они получились именно такими и какие шаги нужны для повышения качества.",
    ]


def forecast_paragraphs() -> list[str]:
    return [
        "Сценарный прогноз в проекте построен как агрегирование новостей по близким темам. Это делает систему полезнее обычного поиска: пользователь видит не только отдельные документы, но и группу сообщений, которые описывают похожий экономический сюжет. Такой подход помогает понять, является ли сигнал единичным событием или повторяется в нескольких публикациях.",
        "Для каждой группы новостей система учитывает оценки трех моделей и распределение классов влияния. Если в группе преобладают negative-сигналы, интерфейс показывает соответствующее направление сценария; если положительные и отрицательные сигналы сбалансированы, вывод становится осторожнее. Это снижает риск слишком уверенной интерпретации по одной новости.",
        "Подключаемая языковая модель используется для формулирования прогноза на естественном языке. При этом она получает полный текст выбранных новостей и рассчитанные ML-сигналы. Поэтому ее роль состоит в аналитическом обобщении фактов, а не в генерации произвольного ответа без опоры на корпус.",
        "С точки зрения курсовой работы этот режим важен, потому что связывает ML-классификацию, retrieval и диалоговую генерацию в один пользовательский сценарий. Прогноз показывает, как результаты моделей могут быть применены не только для метки отдельной новости, но и для объяснения общей картины в группе экономических событий.",
    ]


def retrieval_quality_paragraphs() -> list[str]:
    return [
        "Качество retrieval-компонента влияет на итоговый ответ не меньше, чем качество классификатора. Если поиск возвращает нерелевантные новости, даже сильная языковая модель будет формировать вывод по слабому контексту. Поэтому в RAG-системе важно отдельно анализировать, какие документы попадают в top-k и насколько они действительно отвечают на вопрос пользователя.",
        "В текущей версии retrieval используется как практический компонент поиска источников. Для дальнейшего развития можно ввести ручную разметку релевантности по набору типовых вопросов и считать precision@k. Такая оценка позволит понять, какие запросы работают хуже: общие макроэкономические вопросы, вопросы по компаниям, отраслевые темы или запросы с косвенными формулировками.",
        "Важным элементом пользовательского доверия является отображение найденных источников. Если система показывает только итоговый текст, пользователь не видит, почему был сделан вывод. Если рядом есть новости, score релевантности и ML-сигнал, результат становится проверяемым. Это особенно важно для экономической аналитики, где вывод должен иметь фактическую опору.",
        "RAG-подход в проекте не отменяет необходимость классификации. Retrieval находит документы, но не объясняет, какое влияние они несут. Классификатор добавляет структурированный сигнал, а языковая модель превращает найденные факты и сигналы в связный ответ. Таким образом, три компонента выполняют разные роли и дополняют друг друга.",
    ]


def interface_validation_paragraphs() -> list[str]:
    return [
        "Пользовательский интерфейс в проекте выполняет не только демонстрационную функцию. Он связывает исследовательские результаты с понятным сценарием работы. Страница чата показывает прикладной результат, страница ML-отчета раскрывает качество моделей, а страница прогноза демонстрирует агрегированную аналитику по корпусу.",
        "Такое разделение страниц помогает проверять систему на защите. Если требуется показать тему курсовой, достаточно открыть чат и задать вопрос по экономическим новостям. Если нужно обосновать ML-часть, открывается отчет с метриками, матрицей ошибок и признаками. Если нужно показать использование нескольких новостей по одной теме, используется страница прогноза.",
        "В интерфейсе важно избегать чрезмерной имитации готового промышленного продукта. Курсовая работа должна показывать, как работает метод и какие ограничения он имеет. Поэтому в приложении сохраняются исследовательские элементы: метрики, распределение классов, источники, статусы обработки и возможность сравнить модели.",
        "С точки зрения пользователя, наиболее ценным является связный путь от вопроса до проверяемого вывода. Пользователь вводит запрос, видит найденные источники, получает ответ, а при необходимости переходит к ML-отчету и проверяет качество модели. Такой сценарий делает систему понятной и защищаемой как учебный ML-проект.",
    ]
...
def architecture_quality_paragraphs() -> list[str]:
    return [
        "Слоистая структура сервисов помогает отделить бизнес-логику от инфраструктуры. Доменный слой описывает сущности и правила предметной области, application-слой реализует сценарии использования, infrastructure-слой отвечает за базы, внешние API и очереди, а presentation-слой принимает HTTP-запросы. Такое разделение делает код более поддерживаемым и упрощает замену отдельных компонентов.",
        "Использование Protocol для интерфейсов позволяет описывать зависимости через ожидаемое поведение, а не через конкретные классы. Например, application-слой может требовать абстрактный репозиторий новостей или абстрактный клиент языковой модели. Конкретная реализация подключается снаружи, поэтому сценарий не зависит напрямую от Qdrant, Redis или внешнего API.",
        "Dishka используется для сборки зависимостей. Это важно в микросервисной архитектуре, потому что каждый сервис имеет собственные адаптеры и собственную точку входа, но общие подходы к конфигурации и зависимостям остаются одинаковыми. Благодаря этому в проекте можно поддерживать современную архитектуру без дублирования большого количества технического кода.",
        "Фоновые задачи вынесены отдельно, чтобы тяжелая индексация корпуса не блокировала пользовательский интерфейс. Пользовательский запрос должен обрабатываться быстро, а подготовка данных, переиндексация и расчет отчетов могут выполняться асинхронно. Redis и Taskiq закрывают этот сценарий без добавления более тяжелого брокера сообщений.",
        "Микросервисность в проекте не используется ради количества сервисов. Каждый сервис связан с отдельной ролью в ML/RAG-процессе: новости, поиск, анализ, диалог, API Gateway и интерфейс. Поэтому архитектура помогает объяснить систему на защите: можно показать не только код, но и движение данных между компонентами.",
    ]
...
def model_improvement_plan_paragraphs() -> list[str]:
    return [
        "Улучшение метрик следует выполнять поэтапно. На первом этапе рационально работать с данными: удалить дубликаты, проверить пустые тексты, нормализовать целевые метки и проанализировать спорные примеры. Если данные содержат шум, усложнение модели может не дать устойчивого прироста качества.",
        "На втором этапе нужно улучшать baseline. Для TF-IDF можно подобрать n-граммы, размер словаря, `min_df`, `max_df`, регуляризацию и веса классов. Такой этап сравнительно дешевый по вычислениям, но часто дает заметный прирост качества именно в текстовых задачах с сильными лексическими признаками.",
        "На третьем этапе можно улучшать embedding-подход. Важно выбрать модель представлений, которая лучше подходит для финансовых и экономических текстов, а также проверить нормализацию эмбеддингов. Если embedding лучше отражает смысловую близость новостей, он может усилить не только классификацию, но и retrieval-поиск.",
        "На четвертом этапе имеет смысл переходить к более сильному transformer-классификатору. Для этого нужны веса классов, early stopping, контроль learning rate, достаточное число эпох и сохранение лучшей модели по validation macro F1. Такой подход требует больше ресурсов, но может лучше учитывать контекст слов и сложные формулировки.",
        "На пятом этапе нужно улучшать итоговую систему, а не только отдельную модель. Даже при росте macro F1 важно проверить, стал ли лучше пользовательский ответ: релевантны ли источники, корректно ли формируется прогноз, не теряется ли скорость, понятны ли объяснения. Для диалоговой системы качество модели и качество пользовательского сценария должны оцениваться вместе.",
    ]
...
def build_docx() -> Path:
    ...
    add_paragraphs(doc, economic_domain_paragraphs())
    add_table(
        doc,
    ...
    add_paragraphs(doc, system_flow_paragraphs())
    add_table(
        doc,
    ...
    add_paragraphs(doc, hyperparameter_paragraphs())
    add_paragraphs(doc, evaluation_protocol_paragraphs())
    add_table(
        doc,
    ...
    add_paragraphs(doc, architecture_quality_paragraphs())
    ...
    add_paragraphs(doc, forecast_paragraphs())
    add_paragraphs(doc, retrieval_quality_paragraphs())
    add_paragraphs(doc, interface_validation_paragraphs())
    ...
    add_paragraphs(doc, overfitting_paragraphs())
    add_paragraphs(doc, error_analysis_paragraphs())
    add_paragraphs(doc, validity_paragraphs())
    ...
    add_paragraphs(doc, model_improvement_plan_paragraphs())
    add_bullets(
        doc,
        [
