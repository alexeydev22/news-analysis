...
## Проверка тематического прогноза во фронтенде

Тематический прогноз строится после индексации новостей. В обычном демо
достаточно нажать `Индексировать CSV`, дождаться успешной индексации, затем
нажать `Сформировать прогноз по темам`.

Что делает backend:

- `analysis-service` создает job прогноза;
- `analysis-worker` через Taskiq + Redis выполняет тяжелую работу;
- worker читает проиндексированные документы из `retrieval-service`;
- `retrieval-service` возвращает похожие новости из Qdrant;
- `analysis-service` агрегирует impact-метки выбранного классификатора и
  сохраняет результат в `reports/topic-forecast/latest.json`.

Ожидаемый результат в UI:

- статус меняется с `в очереди` или `выполняется` на `готов`;
- появляется список тем;
- в каждой теме видны связанные новости, общее влияние, уверенность,
  аргументы, риски и осторожный прогноз;
- текст прогноза не является финансовой рекомендацией.
