...
_UNAVAILABLE_MESSAGE = "groq forecast generation is unavailable"


class GroqForecastGenerationError(RuntimeError):
    """Ошибка генерации экономического прогноза через Groq."""
...
class GroqEconomicForecastGenerator:
    ...
    async def generate(self, request: GroqForecastRequest) -> GroqForecastResponse:
        ...
        except GroqForecastGenerationError:
            raise
    ...
    def _select_news_items(
        ...
        if request.scope != GroqForecastScope.NEWS or request.news_id is None:
