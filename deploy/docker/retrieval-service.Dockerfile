...
COPY apps/retrieval-service/pyproject.toml ./apps/retrieval-service/pyproject.toml

RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --package economic-news-retrieval-service --no-dev --no-install-workspace
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --package economic-news-retrieval-service --no-dev
