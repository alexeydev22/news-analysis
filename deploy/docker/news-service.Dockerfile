...
COPY apps/news-service/pyproject.toml ./apps/news-service/pyproject.toml

RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --package economic-news-news-service --no-dev --no-install-workspace
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --package economic-news-news-service --no-dev
