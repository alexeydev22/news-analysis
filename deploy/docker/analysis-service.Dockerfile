...
COPY apps/analysis-service/pyproject.toml ./apps/analysis-service/pyproject.toml

RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --package economic-news-analysis-service --no-dev --no-install-workspace
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --package economic-news-analysis-service --no-dev
